...
layui.define(['laytpl', 'layer'], function (exports) {
        ...
        return $.ajax($.extend({
            ...
            , success: function (res) {
                ...
                if (res[response.statusName] == statusCode.ok) {
                    layer.tips_success(res['msg'],2500,res['url']);
                    ...
                    if (/^5\d\d$/.test(res[response.statusName])) {
                        var time = 2600;
                        if(res[response.statusName]==580)
                            time = 3200;

                        view.tips_warn(res[response.msgName],res['url'],time);
                    } else {
                        view.error(error);
                    }
                }
    ...
    view.tips_warn = function (content, url,time) {
        layer.jumpUrl = url;
        layer.msg(content, {icon: 5, time: isEmpty(time)?2500:time},function(url){
            url = layer.jumpUrl;
            layer.jumpUrl='';
            jumper(url);
        });
    ...
    layer.tips_warn = function (content,time,url) {
        layer.jumpUrl = url;
        layer.msg(content, {
            icon: 2
            ,time: isEmpty(time)?2500:time
        }, function(url){
            url = layer.jumpUrl;
            layer.jumpUrl='';
            jumper(url);
    ...
    layer.tips_success = function (content,time,url) {
        if(isEmpty(content))
            return true;

...
layui.define('view', function(exports){
    var $ = layui.jquery
        ...
        ,admin = {
            ...
            ,tabsBodyChange: function(index, options){
                ...
                if(options.openTabsPage===true){
                    admin.tabsBody(index).find('iframe').attr('src',options.url);
                }
                admin.tabsBody(index).addClass(SHOW).siblings().removeClass(SHOW);
                events.rollPage('auto', index);

                try{
                    var windowNameObj = (isEmpty(window.name)||(window.name.indexOf('{"')!==0))?{}:JSON.parse(window.name);
                    var main_page = $('#LAY_app_tabsheader>li:first').attr('lay-attr');
                    if(options.url!==main_page){
                        windowNameObj.lastTab = options;
                    }else {
                        var homeTab = $('#LAY_app iframe:first');
                        if(isEmpty(homeTab.attr('src'))){
                            homeTab.attr('src',main_page);
                        }
                        windowNameObj.lastTab={};
                    }
                    window.name = JSON.stringify(windowNameObj);
    ...
    $body.on('click', '*[lay-href]', function(){
        ...
        if(href.indexOf('newWindow:')===0){
            window.open(href.split('newWindow:')[1]);
            // othis.parent('.layui-nav-item.layui-this').removeClass('layui-this');
            return;
        }
